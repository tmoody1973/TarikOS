...
- **Don't** bring in a component library for visual primitives; the system is hand-rolled Tailwind on the token set above. **One scoped exception, agreed 2026-08-11: the Studio editor.** `/studio/[documentId]` renders Plate's own components from its shadcn registry — floating toolbar, slash menu, drag handles, tables, lists, media, DOCX export. A hand-rolled imitation of a rich-text editor is permanently one feature behind, and a writing surface is judged against Notion and Word rather than against a dashboard. The exception stops at the editor's frame: the page around it, the index, the history panel and every other surface stay hand-rolled. Do not widen it, and do not "correct" the editor back.
