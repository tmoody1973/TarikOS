...
## What changed on audit day

Phase 1 was done the same day. CI now runs typecheck, tests, lint and build on
every pull request and every push to `main` — proven by deliberately breaking a
...
change would tax a workflow that works. Revisit if anyone else commits, or if a
bad deploy ever actually bites.
